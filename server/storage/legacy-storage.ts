...
export class MemStorage implements IStorage {
  ...
  async createPlayer(insertPlayer: InsertPlayer): Promise<Player> {
    const player: Player = {
      ...
      position: insertPlayer.position ?? undefined,
      game_logs: (insertPlayer as any).game_logs ?? undefined,
      team_pace: (insertPlayer as any).team_pace ?? undefined,
      on_off_splits: (insertPlayer as any).on_off_splits ?? undefined,
      next_game_location: (insertPlayer as any).next_game_location ?? undefined,
      next_opponent: (insertPlayer as any).next_opponent ?? undefined,
      usage_rate: (insertPlayer as any).usage_rate ?? undefined,
      season_averages: {
  ...
  async createPotentialBet(bet: InsertPotentialBet): Promise<PotentialBet> {
    ...
    const newBet: PotentialBet = {
      ...
      sample_size: bet.sample_size ?? undefined,
      adjusted_hit_rate: bet.adjusted_hit_rate ?? undefined,
  ...
  async updatePotentialBet(id: number, updates: Partial<InsertPotentialBet>): Promise<PotentialBet> {
    ...
    const updatedBet: PotentialBet = {
      ...
      sample_size: (updates.sample_size ?? existingBet.sample_size) ?? undefined,
      adjusted_hit_rate: (updates.adjusted_hit_rate ?? existingBet.adjusted_hit_rate) ?? undefined,
...
// Storage instance is now exported from ./index.ts
