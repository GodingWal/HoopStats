...
cmd = '''cd /var/www/courtsideedge && cat .env | grep -E "PUPPETEER|SCRAPER|PROXY" 2>/dev/null || echo "No matching env vars found"'''
stdin, stdout, stderr = client.exec_command(cmd, timeout=30)
print(stdout.read().decode('utf-8', errors='replace'))

# Check if puppeteer is installed
print('\n=== Checking Puppeteer Availability ===')
cmd = '''cd /var/www/courtsideedge && npm list puppeteer 2>&1 | head -5'''
stdin, stdout, stderr = client.exec_command(cmd, timeout=30)
print(stdout.read().decode('utf-8', errors='replace'))
...
cmd = 'pm2 logs courtsideedge --nostream --lines 50 2>&1 | grep -E "429|blocked|ScraperAPI|Puppeteer" | tail -20'
stdin, stdout, stderr = client.exec_command(cmd, timeout=30)
print(stdout.read().decode('utf-8', errors='replace'))
